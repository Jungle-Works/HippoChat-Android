...
public class IncomingRinger {
    ...
    public void start(@Nullable Uri uri, boolean vibrate) {
        ...
        if (shouldVibrate(context, player, ringerMode, vibrate)) {
            HippoLog.i(TAG, "Starting vibration");
        ...
        if (player != null && ringerMode == AudioManager.RINGER_MODE_NORMAL) {
            try {
                if (!player.isPlaying()) {
                    player.prepare();
                    player.start();
                    HippoLog.i(TAG, "Playing ringtone now...");
                } else {
                    HippoLog.w(TAG, "Ringtone is already playing, declining to restart.");
                }
            ...
            HippoLog.w(TAG, "Not ringing, mode: " + ringerMode);
        }
    }

    public void stop() {
        if (player != null) {
            HippoLog.i(TAG, "Stopping ringer");
            player.release();
            player = null;
        }

        HippoLog.i(TAG, "Cancelling vibrator");
        vibrator.cancel();
    }
    ...
    private MediaPlayer createPlayer(@NonNull Uri ringtoneUri) {
        try {
            ...
            HippoLog.e(TAG, "Failed to create player for incoming call ringer");
            return null;
        }
    }
    ...
    private class MediaPlayerErrorListener implements MediaPlayer.OnErrorListener {
        ...
        public boolean onError(MediaPlayer mp, int what, int extra) {
            HippoLog.w(TAG, "onError(" + mp + ", " + what + ", " + extra);
            player = null;
            return false;
        }
    }
